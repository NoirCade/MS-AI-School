...
optimizer = torch.optim.AdamW(finetune_net.parameters(),
                              lr=0.0003, weight_decay=0.0005)   # 데이터 수가 매우 적어서 lr, weight_decay 아주 낮게 설정
...
for epoch in range(num_epochs):
    ...
    print(f'Epoch [{epoch+1}/{num_epochs}] Train Loss: {(running_loss / train_steps):.3f} '
          f'Train Accuracy: {train_accuracy:.3f} Vali Accuracy: {val_accuracy:.3f}')
    ...
    if epoch == num_epochs-1:
        Accuracydf.to_csv('./modelAccuracy.csv', index=False)
