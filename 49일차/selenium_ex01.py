...
def create_folder(directory):
    try:
        if not os.ppath.exists(directory):
...
# keword = driver.find_element_by_xpath('/html/body/div[1]/div[3]/form/div[1]/div[1]/div[1]/div/div[2]/input')
# keword.send_keys(keywords)
...
for index, i in enumerate(links):
    ...
    urllib.request.urlretrieve(
        url, 'img/' + keywords + '_img_download' + keywords + '_' + str(index) + '.jpg')
    print(str(index) + '/' + str(len(links)) + keywords + ' 다운로드 시간 ----- : ',
