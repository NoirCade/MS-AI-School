...
def selenium_scroll_option(driver):
    SCROLL_PAUSE_SEC = 10    # 스크롤 대기 시간

    # 스크롤 높이를 가져온다 -> 동적 웹페이지이기 때문에 스크롤을 옮겨야 데이터가 로드된다.
...
def main():
    # option = Options()
    # option.add_argument(
    #     'UserAgent에서 받아온 주소 입력'
    # )
    os.makedirs('search_data', exist_ok=True)
    search_list = input('검색어 입력(,로 구분하여 입력): ')
    N = input('수집할 데이터 개수: ')
    search_list = [i.strip() for i in search_list.split(',')]   # 서치 리스트에 담음
    # 브라우저 로딩을 고려하여 대기
    # time sleep과는 다르게 로딩이 끝나면 바로 이후 실행
